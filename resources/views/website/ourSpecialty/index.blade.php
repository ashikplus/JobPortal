...
<div class="page-content">
    ...
    <div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-road"></i>@lang('english.OUR_SPECIALITY')
            </div>
            <div class="actions">
                <a class="btn btn-default btn-sm create-new" href="{{ URL::to('ourSpecialty/create'.Helper::queryPageStr($qpArr)) }}"> {{trans('english.CREATE_NEW')}}
                        ...
                        <tr>
                            ...
                            <td class="text-center vcenter">
                                <?php if (!empty($target->featured_image)) { ?>
                                    <img width="70" height="70" src="{{URL::to('/')}}/public/uploads/website/{{$target->featured_image}}" alt="{{ $target->title}}"/>
                                <?php } else { ?>
                                    <img width="70" height="70" src="{{URL::to('/')}}/public/img/no-image.png" alt="{{ $target->title}}"/>
                                <?php } ?>
                            </td>
                            ...
                            <td  class="text-center vcenter">
                                <div>
                                    {{ Form::open(array('url' => 'ourSpecialty/' . $target->id, 'id'=>'delete')) }}
                                    {{ Form::hidden('_method', 'DELETE') }}
                                    <a class="btn btn-icon-only btn-primary tooltips" title="Edit" href="{{ URL::to('ourSpecialty/' . $target->id . '/edit'.Helper::queryPageStr($qpArr)) }}">
