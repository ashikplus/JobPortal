...
if (!empty($data->nigotiable)) {
    ...
    if (!empty($data->salary_from) && !empty($data->salary_to) || !empty($data->salary_to)) {
        if (empty($data->salary_from)) {
            $salaryRange = "0 to " . $data->salary_to;
        } else {
            $salaryRange = $data->salary_from . " to " . $data->salary_to;
        }
    } else {
        $salaryRange = $data->salary_from;
    }
}
?>
...
</script><?php /**PATH D:\xampp\htdocs\dysin\resources\views/circular/circularDetailsModal.blade.php ENDPATH**/ ?>